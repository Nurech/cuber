...
ERNO.renderers.CSS3DCubelet.methods = (function(){
	...
	return {
		...
		//  Visual switches.
		getFaceElements: function ( face,selector ){

			var selectorString = selector || '';
			return Array.prototype.slice.call( this.css3DObject.element.querySelectorAll( '.face' + face + selectorString ));
     ...
     * Added this
     */
    changeFaceColor: function(face, colorId){
      var colors = ['gray', 'green', 'yellow', 'blue', 'red', 'orange', 'white', 'colorless', 'stickerLogo']
      face = face === undefined ? '' : face
      this.getFaceElements( face,' .sticker' ).forEach( item => {
        colors.forEach(color => item.classList.remove(color))
        item.classList.add(colors[colorId])
      });
    },
